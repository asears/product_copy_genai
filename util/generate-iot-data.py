...
df_spec = (
    ...
    .withColumn(
        "timestamp", LongType(), minValue=1577833200, maxValue=1673714337, random=True
    )
    .withColumn(
        "sensor_1",
        IntegerType(),
        ...
        maxValue=100,
        random=True,
        distribution=dist.Gamma(40.0, 9.0),
    )
...
kafka_ready_df = df.select(
    ...
    to_json(struct([col(column) for column in df.columns]))
    .cast(BinaryType())
    .alias("value"),
)
...
# Write to Kafka
(kafka_ready_df.write.format("kafka").options(**options).save())
